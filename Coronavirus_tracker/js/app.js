...
const currentDatePlaceholder = document.querySelector(
...
async function listCountries() {
  ...
  async function printCountryStats(name) {
    ...
    countryRecovered.innerText = `Recovered: ${(
      country_data.confirmed.value - country_data.deaths.value
    ).toLocaleString()}`;
    countryDeaths.innerText = `Deaths: ${country_data.deaths.value.toLocaleString()}`;

    //Check if card has children, to remove previous stats
    if (card.children) {
      card.innerHTML = "";
    }
